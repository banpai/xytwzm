...
//根据名称关闭地图事件
MapManager.prototype.closeEvent = function(name) {
...
//根据关闭所有地图事件
MapManager.prototype.closeAllEvent = function() {
	for (var i = 0; i < this.events.length; i++) {
		var e = popomap.events[i];
		e.close = 1;
	}
}
//添加地图事件
MapManager.prototype.addEvent = function(event,zoomNum) {
	for (var i = 0; i < this.events.length; i++) {
		var e = popomap.events[i];
		if (e.name == "grid_labels_event") {
			e.close = event.close;
			return;
		}
	}
	if(typeof event.zoom!="undefined"&&event.zoom!=null){
		for(var i=0;i<event.zoom.length;i++)
			event.zoom[i]+=zoomNum;
	}
	this.events.push(event);
	this.map.events.register(event.type, event, event.listener);
}
//批量添加地图事件
MapManager.prototype.addEvents = function(events,zoomNum) {
	for (var i = 0; i < events.length; i++) {
		this.addEvent(events[i],zoomNum);
	}
}
...
/** 初始化地图管理控件 */
MapManager.prototype.init = function($map, $marktype, $styleMapOpt,$featurestyle) {
	...
	if(typeof($marktype) != 'undefined' && $marktype != null && $marktype != ''){ // 如果指定了标注绘制类型
		...
		var style;
		if(typeof($featurestyle) != 'undefined' && $featurestyle != null && $featurestyle != '')
			style=$featurestyle;
		else
		style = {
				fill : true,
...
MapManager.prototype.setShowLayerStyle = function($styleMapOption){
	if(this.map == null || typeof(this.map) == 'undefined') return;
	if (this.showlayer == null || typeof(this.showlayer) == 'undefined') return;
	var style = $.extend({
		fill : true,
	...
	this.showlayer.styleMap = styleMap;
}
...
// 继续画图
MapManager.prototype.choiceLayerViews = function($value) {
	if(this.modifyControl != null) this.deactivateModifyControl(); // 禁用拖拽控制器
...
/** 根据传入的位置与标注类型创建标注 */
MapManager.prototype.drawFeaturesByCoord = function(objX, objY, marktype, pid,style,attributes){
	if (this.showlayer == null || typeof(this.showlayer) == 'undefined') return;
	...
	if(typeof style !='undefined'&&style!=null)
    	feature = new OMAP.Feature.Vector(object,attributes,style);
    else
    	feature = new OMAP.Feature.Vector(object,attributes);
	feature.attributes.pid = typestr + pid;
	//feature.attributes.title = "";
	this.showlayer.addFeatures([feature]);
}
/** 根据传入的位置与标注类型创建标注 */
MapManager.prototype.drawFeatures = function(features){
	if (this.showlayer == null || typeof(this.showlayer) == 'undefined') return;
	this.showlayer.addFeatures(features);
}
/** 创建文本Feature标注 */
